...
void mostrarEventosAdmin(vector<Evento>& eventos) {
    int totalEventos = 0;
    ...
    if (totalEventos == 0) {
        moverCursor(10, 5);
        cout << "No se registraron eventos de administrador." << endl;
        return;
    }
    ...
    string titulo = "\tESTADÍSTICAS DE EVENTOS PARA EL ADMINISTRADOR";
...
int main() {
    ...
    // Leer el archivo y extraer solo los eventos de administrador
    while (getline(archivo, linea)) {
        ...
        if (pos != string::npos) {
            ...
            // Mostrar solo los eventos de administrador
    ...
    mostrarEventosAdmin(eventos); // Llamar a la función que muestra los eventos de administrador
