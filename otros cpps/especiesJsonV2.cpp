...
#include "extraerBioma.h"
#include "json.hpp"

using json = nlohmann::json;
using namespace std;

const string animal = "animales.json";
const string vegetal = "vegetal.json";
const string bioma = "bioma.json";
...
Especie extraerDatosEspecie() {
    json especies;
    string nombreBuscado, nomb;
    Especie especieEncontrada; // Variable para la estructura

    int ops = seleccionarTipoEspecie();
    
    if(ops==3){
    	cout << "Ingresa el nombre del bioma: ";
    	fflush(stdin);
	    getline(cin, nomb);
    	extraerBioma(nomb);
	}
